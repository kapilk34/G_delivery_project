  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 py-10">
      ...
      <div className="w-[95%] sm:w-[90%] md:w-[85%] lg:w-[80%] mx-auto relative">
        ...
        <Link 
          href={"/"} 
          className="flex items-center gap-2 text-green-700 hover:text-green-900 font-semibold transition-all mb-6"
        >
          <ArrowLeft size={20}/>
        ...
        {cartData.length === 0 ? (
          <div className='text-center py-20 bg-white rounded-3xl shadow-xl border border-green-100'>
            ...
            <Link 
              href={"/"} 
              className='bg-green-600 text-white px-8 py-3 rounded-full hover:bg-green-700 transition-all inline-block font-medium shadow-md hover:shadow-lg'
            >
              Continue Shopping
            </Link>
          </div>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-10'>
            ...
            <div className='lg:col-span-2 space-y-6'>
              {cartData.map((item, index) => (
                <div 
                  key={index} 
                  className='flex flex-col sm:flex-row items-center bg-white rounded-3xl shadow-md hover:shadow-xl p-6 transition-all duration-300 border border-gray-100'
                >
                  ...
                  <div className='flex items-center gap-3 mt-4 sm:mt-0 bg-green-50 px-4 py-2 rounded-full shadow-inner'>
                    <button 
                      className="w-8 h-8 flex items-center justify-center rounded-full bg-white hover:bg-green-200 transition-all shadow"
                  ...
                  {/* Remove Button */}
                  <button 
                    className='sm:ml-6 mt-4 sm:mt-0 text-red-500 hover:text-red-700 transition-all'
            ...
            {/* Order Summary (UI Only – No Logic Changed) */}
            <div className='bg-white rounded-3xl shadow-xl p-8 h-fit border border-green-100'>
              ...
              <div className='border-t pt-4 mt-4 flex justify-between text-lg font-bold text-green-700'>
                <span>Total</span>
                <span>
                  ₹{cartData.reduce((acc, item) => acc + Number(item.price) * item.quantity, 0)}
                </span>
              </div>

              <button className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all font-medium shadow-md hover:shadow-lg'>
                Proceed to Checkout
              </button>
            </div>
