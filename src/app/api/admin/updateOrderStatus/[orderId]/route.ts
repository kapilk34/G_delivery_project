...
export async function POST(
    ...
    try {
        ...
        if (status === "Out of Delivery" && !order.assignment) {
            ...
            console.log(
                `Found ${nearByDeliveryBoys.length} delivery boys nearby for order`,
                orderId
            );
            ...
            if (nearByDeliveryBoys.length === 0) {
                console.log(
                    "No delivery boys found by location, checking all delivery boys..."
                );
            ...
            if (candidates.length === 0) {
                ...
                return NextResponse.json(
                    {
                        message:
                            "There are no available Delivery Boys. Please try again later.",
                    },
                    { status: 200 }
                );
            }
            ...
            // Get available boys data
            let availableDeliveryBoys =
                nearByDeliveryBoys.length > 0
                    ? nearByDeliveryBoys.filter((b) =>
                          candidates
                              .map((c: any) => String(c))
                              .includes(String(b._id))
                      )
